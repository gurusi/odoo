...
VERSION=14.0_iot_bpi
VERSION_IOTBOX=21.04
REPO=https://github.com/gurusi/odoo.git
...
#     # unzip 2020-04-19-raspbian-jessie-bpi-m2u-sd-emmc.img.zip
#     # mv 2020-04-19-raspbian-jessie-bpi-m2u-sd-emmc.img raspios.img
#     unxz Armbian_23.02.2_Bananapim5_bullseye_current_6.1.11.img.xz
#     mv Armbian_23.02.2_Bananapim5_bullseye_current_6.1.11.img raspios.img

# fi


RASPIOS=$(echo *raspios*.img)
RASPIOS="Armbian_23.05.0-trunk_Bananapim2ultra_buster_current_6.1.24.img"

rsync -avh --progress "${RASPIOS}" iotbox.img
# mv "${RASPIOS}" iotbox.img
...
dd if=/dev/zero bs=1M count=4096 status=progress >> iotbox.img

# resize partition table
echo "Fdisking"

SECTORS_BOOT_START=$(sudo fdisk -l iotbox.img | tail -n 1 | awk 'NR==1 {print $2}')
...
# cleanup
umount -fv "${MOUNT_POINT}"/dev
umount -fv "${MOUNT_POINT}"/proc
